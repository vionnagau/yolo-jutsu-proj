...
VFX_CONFIG = {
    ...
    "Chidori (LIGHTNING BLADE)": {"type": "hand", "path": "chidori_frames", "secondary": None},
...
class JutsuVFX:
    def __init__(self, name, config):
        ...
        # We're pre-loading secondary assests to any potential avoid I/O lag when rendering
...
def overlay_effect(bg, jutsu_name, x=None, y=None, size=350, sw=900, sh=450):
    ...
    if vfx.type == "hand":
        ...
        if vfx.primary_finished: is_finished = True
        else: res_frame = _apply_alpha_overlay(bg, effect_img, sw//2, sh//2, sw=sw, sh=sh, is_full=True)
